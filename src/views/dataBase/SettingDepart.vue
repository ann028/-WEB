          ...
          <el-table
            ...
            <el-table-column label="操作" align="center" width="150">
              <template slot-scope="scope">
                ...
                <span style="cursor: pointer;color:#5792FC;" @click.stop="addGroup(scope.row)">添加组长</span>
                <span style="cursor: pointer;color:#5792FC;margin-left:5px" @click.stop="addMember(scope.row)">添加组员</span>
              </template>
            </el-table-column>
          </el-table>
        </section>
        <section class="col selected_box">
          <section class="box_title cross_center">组长</section>
          ...
          <section class="box_title cross_center">组员</section>
            <section class="selected_list flex1 cross_center col">
            <section class="user_item mb10 center" v-for="(item, index) in selectedMember" :key="index">
              {{item.departmentName}}-{{item.name}} 
              <section class="item_del center" @click="delMember(index)">-</section>
            </section>
          </section>
        </section>
      </section>
...
<script lang="ts">
  ...
  export default class UserAddModal extends Vue {
    ...
    @Prop({}) public readonly selectObj!: any
    @Prop({}) public readonly addUserlist!: any
    // @Prop(Number) private readonly roleId!: number
    private selectedUser: any[] = []
    private selectedGroup: any[] = this.addUserlist['GROUP']||[]
    private selectedMember: any[] = this.addUserlist['MEMBER']||[]
    private searchName = ''
    private userList = [];
    ...
    public save() {
      this.$emit('save', this.selectedGroup,this.selectedMember)
    }
    ...
    private addGroup(row: any) {
      if (this.selectedGroup.every((ele: any) => {
        ...
        if (this.selectedMember.every((ele: any) => {
          return ele.id !== row.id
        })) {
          this.selectedGroup.push(row)
        }else {
          this.$message.warning(`${row.name} 只具有一种身份`)
        }
      } else {
        this.$message.warning(`${row.name} 已经在已选用户中`)
      }
    }
    private addMember(row: any) {
      if (this.selectedMember.every((ele: any) => {
        return ele.id !== row.id
      })) {
         if (this.selectedGroup.every((ele: any) => {
    ...
    // }
     ...
     private delMember(index: number) {
    ...
    private changePageValue(page: string) {
      this.pageJson.pageSize = parseInt(page) * 10
