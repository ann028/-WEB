  ...
  <main  class="container">
    ...
    <div style="display:flex;justify-content:space-between;">
      <section class="flex" style="flex-wrap:wrap">
        ...
        <section v-if="tab === '工作'" class="flex mt20">
        ...
        <section v-if="tab === '工作'" class="flex mt20" style="flex-shrink:0">
          ...
          <el-select v-model="searchInfo.level" placeholder="">
      ...
      <section class="flex">
        <button
          ...
          v-if="permission.includes('prm1424')"
        >
        ...
        <el-tab-pane label="工作" name="工作">
          <el-table
            ...
            >
              ...
              <el-table-column label="操作" align="center" class="main_center" width="150">
                <template slot-scope="scope">
                  <span
                    @click.stop="toDetail(scope.row, 'edit')"
                    style="color:#5792FC;cursor: pointer;margin-right: 5px"
                    v-if="permission.includes('prm1422')"
                  >
                  ...
                  <span
                    ...
                    v-if="permission.includes('prm1423')"
                  >
...
export default class ClientDepartList extends Vue {
  private message = ['直销日报']
  private imgUrl = require('../../assets/img/head/client.png')
  private tab: any = sessionStorage.getItem('tab') || '工作'
  ...
  private async getSellinfo() {
    ...
    // this.searchInfo.timeValue[0] = time ? time.split(',')[0] : this.searchInfo.timeValue[0]
    // this.searchInfo.timeValue[1] = time ? time.split(',')[1] : this.searchInfo.timeValue[1]
    // const current: any = sessionStorage.getItem('currentPage') || 1
    this.pageJson.currentPage = this.pageJson.currentPage  * 1
    ...
    if (this.tab === '工作') {
      ...
      const res: any = await client.getClientBydepart(data)
      this.tableData = res.data.data.records
      this.pageJson.total = res.data.data.total
    } else if (this.tab === '请假') {
      daily.getdepartLeave({
  ...
  private toDetail( info: any, type: any) {
    if (this.tab === '工作') {
      if (type === 'edit') {
        this.$router.push({ name: 'clientdailyAdd', query: { id: info.id, operation: 'edit', type: '工作' }})
      } else {
          if (this.$store.state.permission.includes('prm1421')) {
             this.$router.push({ name: 'clientdailyAdd', query: { id: info.id, operation: 'detail', type: '工作' }})
          }
      }
    } else if (this.tab === '请假') {
      if (type === 'edit') {
        this.$router.push({name: 'clientdailyAdd', query: {id: info.id, operation: 'edit', type: '请假'}})
      } else {
          if (this.$store.state.permission.includes('prm1421')) {
  ...
  private async deleteDetail1(id: any) {
    if (this.tab === '工作') {
      const res: any = await client.deleteClientDaily(`id=${id}`)
  ...
  private async changeState(row: any) {
      ...
      if (res.data.success) {
          ...
          this.$router.push({name: 'clientDepartList'})
      } else {
          this.$message({
