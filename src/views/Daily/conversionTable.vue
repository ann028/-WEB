<template>
  <main class="container">
    ...
    <div  style="margin-top: 20px;display: flex;justify-content: space-between;">
      ...
      <section class="flex">
        <button v-if="permission.includes('prm1233')" @click="exportConversion"  class="btn btn_warning" style="width: 76px;height: 32px;">
...
export default class actualTable extends  Vue {
  private message = ['日常管理', '周报', '统计表', '折算表'];
  private tableData = []
  private searchInfo = {
  ...
  // 导出
  private async exportConversion() {
    let res: any
    if (this.$store.state.permission.includes('prm1232')) {
      res = await daily.exportConversion({
        endTime: this.searchInfo.timeValue[1],
        startTime: this.searchInfo.timeValue[0],
        userName: this.searchInfo.userName
      })
    } else {
      res = await daily.exportConversionP({
        endTime: this.searchInfo.timeValue[1],
        startTime: this.searchInfo.timeValue[0],
        userName: this.searchInfo.userName
      })
    ...
    downloadElement.download = `折算表-${this.searchInfo.timeValue[0]}-${this.searchInfo.timeValue[1]}.xls`; // 下载后文件名
